...
def diag(X):
    return Expr.vstack([X[i,i] for i in range(X.shape[0])])

def weber(demands, p, ineq=None):
    n = len(demands)
    dim = len(demands[0])
    ...
    currenty = np.reshape(y.level(), (p,dim))
    ...
    info = {
        ...
        "constraints" : M.getSolverIntInfo("optNumcon"),
        "optimalStatus" : M.getPrimalSolutionStatus() == SolutionStatus.Optimal
    }
...
#    inequalities[k] = False

sol, info = weber(demands, p, inequalities)
print(sol)
"""